...
public class Ponto
{
    ...
    public void scale(Ponto dPos, double scale)
    {
        this.x = dPos.getX() + (this.x - dPos.getX()) * scale;
        this.y = dPos.getY() + (this.y - dPos.getY()) * scale;
    }

    public void rotate(Ponto dPos, double angle) {
        double rad = angle * Math.PI / 180;
        double cos = Math.cos(rad);
        double sin = Math.sin(rad);

        double xRel = this.x - dPos.x;
        double yRel = this.getY() - dPos.getY();

        double xRot = xRel * cos - yRel * sin;
        double yRot = yRel * cos + xRel * sin;
        this. x = xRot + dPos.getX();
        this.y = yRot + dPos.getY();
    }
}